...
class SALMONN(nn.Module):
    ...
    def init_speech_Qformer(cls, num_query_token, speech_width, num_hidden_layers=2):
        path="/mnt/data/cgy/bert-base-uncased"
        encoder_config = BertConfig.from_pretrained(path)
        ...
        with self.maybe_autocast():
            if self.use_speech_Qformer:
                ...
                if self.window_level_Qformer:
                    ...
                    speech_embeds_overlap = F.unfold(speech_embeds_tr, kernel_size=kernel, dilation=1, padding=0,
                                                     stride=stride)
    ...
    def encode_speech(self, spectrogram, raw_wav=None, audio_padding_mask=None):
        with self.maybe_autocast():
            ...
            if self.beats_path and raw_wav is not None:
                audio_embeds, _ = self.beats.extract_features(raw_wav, padding_mask=audio_padding_mask,
                                                              feature_only=True)
            else:
                audio_embeds = None
        ...
        if prompt:
            if multi_prompt:
                ...
                p_before_embeds = self.llama_model.model.embed_tokens(
                    p_before_tokens.input_ids) if not self.lora else self.llama_model.model.model.embed_tokens(
                ...
                p_after_embeds = self.llama_model.model.embed_tokens(
                    p_after_tokens.input_ids) if not self.lora else self.llama_model.model.model.embed_tokens(
    ...
    def forward(self, samples, verbose=False):
        ...
        speech_embeds, speech_atts = self.encode_speech(spectrogram, raw_wav=raw_wav,
        ...
        to_regress_embeds = self.llama_model.model.embed_tokens(
            to_regress_tokens.input_ids) if not self.lora else self.llama_model.model.model.embed_tokens(
        ...
        bos_embeds = self.llama_model.model.embed_tokens(
            bos) if not self.lora else self.llama_model.model.model.embed_tokens(bos)
